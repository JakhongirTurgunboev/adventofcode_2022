    ...
    for i in range(len(list_new)):
        if i%2 == 0:
            if int(list_new[i+1][0]) <= int(list_new[i][0]) and int(list_new[i+1][1]) >= int(list_new[i][1]):
                ...
                totalScore += 1
            elif int(list_new[i+1][0]) <= int(list_new[i][0]) and int(list_new[i+1][1]) >= int(list_new[i][0]):
                totalScore += 1
            elif int(list_new[i+1][0]) >= int(list_new[i][0]) and int(list_new[i+1][0]) <= int(list_new[i][1]):
                totalScore += 1
    print(totalScore)
